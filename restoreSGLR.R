restoreSGLR_CCLE<-function(pathwayName,dataCombine,KK = c(1:24)){
  ...
  for(kk in KK){
    ...
    load(filename1)  
    filename = paste("~/Result_priorIncorporateStepwiseRegressionNew_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/restoredPriorIncorporated_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
      ...
      resultsScale <- foreach(k = 1:length(foldIndices)) %dopar% {      
        ...
        foldModel$customTrain(filteredFeatureDataScaled[-foldIndices[[k]],], filteredResponseDataScaled[-foldIndices[[k]]], alpha = 0.5, nfolds = 5,penalty.factor = resultSTEP[[k]]$penalty)
...
restoreSGLR_Sanger<-function(pathwayName,dataCombine,KK = sort(c(103,14,129,1,39,90,127,123,13,4,110,6,9,12,95,94,108,11,17,2,122,124,126,87,84,75,41,82))){
  ### DEMO Stepwise grouping Lasso
  require(predictiveModeling)
  require(synapseClient)
  # synapseLogin("in.sock.jang@sagebase.org","tjsDUD@")
  ...
  for(kk in KK){
    ...
    load(filename1)  
    filename = paste("~/Result_priorIncorporateStepwiseRegressionNew_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/restoredPriorIncorporated_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
