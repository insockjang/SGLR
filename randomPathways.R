randomPathway_CCLE<-function(pathwayName,dataCombine,KK = c(1:24),mcCoreNum = 32){
  ...
  for(kk in KK){  
    filename = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/PriorRandomPathwayStructure_cvDrug_",kk,".Rdata",sep = "")
      ...
      resultsScale <- foreach(k = 1:length(foldIndices)) %dopar% {
        ...
        if(length(which(resultSTEP[[k]]$penalty==0))==0){
          ...
          myRand<-function(pp){
            ...
            foldModel$customTrain(filteredFeatureDataScaled[-foldIndices[[k]],which(penalty_vector==0)], filteredResponseDataScaled[-foldIndices[[k]]], alpha = 0, nfolds = 5)
            
            res <- list(trainPredictions = foldModel$customPredict(filteredFeatureDataScaled[-foldIndices[[k]],which(penalty_vector==0)]), 
                        trainObservations = filteredResponseDataScaled[-foldIndices[[k]]],
                        testPredictions = foldModel$customPredict(filteredFeatureDataScaled[foldIndices[[k]],which(penalty_vector==0)]),
...
randomPathway_Sanger<-function(pathwayName,dataCombine,KK = NA,mcCoreNum = 32){
  ...
  # testfunction<-function(kk){
  for(kk in KK){  
    filename = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/PriorRandomPathwayStructure_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
      #########################################################################################################
      ######## Training and Testing data are scaled(normalized) vs. raw(unnormalized) #######################
      #########################################################################################################
