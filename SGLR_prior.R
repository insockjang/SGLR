SGLR_prior_CCLE<-function(pathwayName,dataCombine,KK=c(1:24),mcCoreNum = 32){
  ...
  source("~/SGSR_new/parallel_stepwiseDecision.R")
  ...
  for(kk in KK){
    filename = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/PriorIncorporated_cvDrug_",kk,".Rdata",sep = "")
...
SGLR_prior_Sanger<-function(pathwayName,dataCombine,KK=NA,mcCoreNum = 32){
  ### DEMO Stepwise grouping Lasso
  require(predictiveModeling)
  require(synapseClient)
  # synapseLogin("in.sock.jang@sagebase.org","tjsDUD@")
  ...
  for(kk in KK){
    filename = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/PriorIncorporated_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
      #########################################################################################################
      ######## Training and Testing data are scaled(normalized) vs. raw(unnormalized) #######################
      #########################################################################################################
