randomGene_CCLE<-function(pathwayName,dataCombine,KK = c(1:24),mcCoreNum = 32){
  ...
  for(kk in KK){
    filename = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/restoredRandom_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
      ...
      load(paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/PriorIncorporated_cvDrug_",kk,".Rdata",sep = ""))
      ...
      resultsScale <- foreach(k = 1:length(foldIndices)) %dopar% {
        if(length(which(resultSTEP[[k]]$penalty==0))==0){
          load(paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/restoredPriorIncorporated_cvDrug_",kk,".Rdata",sep = ""))
          results<-list()
          for(pp in 1:50){
          ...
          myRand<-function(pp){
            set.seed(pp)
            aa<-which(resultSTEP[[k]]$penalty == 0)
            bb<-sample(length(resultSTEP[[k]]$penalty),length(aa))
            foldModel <- myEnetModel1$new()      
            foldModel$customTrain(filteredFeatureDataScaled[-foldIndices[[k]],bb], filteredResponseDataScaled[-foldIndices[[k]]], alpha = 0, nfolds = 5)
            
            res <- list(trainPredictions = foldModel$customPredict(filteredFeatureDataScaled[-foldIndices[[k]],bb]), 
                        trainObservations = filteredResponseDataScaled[-foldIndices[[k]]],
                        testPredictions = foldModel$customPredict(filteredFeatureDataScaled[foldIndices[[k]],bb]),
...
randomGene_Sanger<-function(pathwayName,dataCombine,KK = NA,mcCoreNum = 32){
  ### DEMO Stepwise grouping Lasso
  require(predictiveModeling)
  require(synapseClient)
  require(devtools)
  ...
  for(kk in KK){
    filename = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/restoredRandom_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
      #########################################################################################################
      ######## Training and Testing data are scaled(normalized) vs. raw(unnormalized) #######################
      #########################################################################################################
      ...
      load(paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/PriorIncorporated_cvDrug_",kk,".Rdata",sep = ""))
      ...
      resultsScale <- foreach(k = 1:length(foldIndices)) %dopar% {
        if(length(which(resultSTEP[[k]]$penalty==0))==0){
          load(paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/restoredPriorIncorporated_cvDrug_",kk,".Rdata",sep = ""))
          results<-list()
          for(pp in 1:50){
            results[[pp]]<-resultsScale[[k]]
          }
