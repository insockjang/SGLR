ENetSameNumber_CCLE<-function(pathwayName,dataCombine,ALPHA = 0.5){
  ...
  allProcess<-function(kk){  
    filename1 = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/CCLE/",pathwayName,"/PriorENet_alpha_",ALPHA,"_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename1)){
      ...
      processer<-function(k){                
        ...
        num.of.features<-length(which(resultSTEP[[k]]$penalty ==0))
...
ENetSameNumber_Sanger<-function(pathwayName,dataCombine,ALPHA = 0.5){
  ### Cross training and testing
  library(predictiveModeling)
  library(synapseClient)
  synapseLogin("in.sock.jang@sagebase.org","tjsDUD@")
  ...
  # testfunction<-function(kk){
  allProcess<-function(kk){  
    filename1 = paste("~/Result_priorIncorporateStepwiseRegression_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/PriorENet_alpha_",ALPHA,"_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename1)){
      #########################################################################################################
      ######## Training and Testing data are scaled(normalized) vs. raw(unnormalized) #######################
      #########################################################################################################
